...
def upload_invoice():
    ...
    def generate():
        try:
            ...
            yield f"data: {json.dumps({'step': 1, 'status': 'running', 'title': 'Ingesting PDF', 'detail': f'Processing file: {filename}'})}\n\n"
            time.sleep(0.5)
            
            # Step 2: AI Extract
            yield f"data: {json.dumps({'step': 2, 'status': 'running', 'title': 'AI Extraction', 'detail': 'Analysing the invoice...'})}\n\n"
            
            ai_engine = AIEngine()
            extracted = ai_engine.extract_invoice(file)
            
            # If extraction failed or returned None, use mock data
            if not extracted or not extracted.get('vendor_name'):
                extracted = {
                    'vendor_name': 'Matahari Trading Sdn Bhd',
                    'invoice_no': f'INV-{datetime.now().strftime("%Y%m")}-{random.randint(100, 999)}',
                    'invoice_date': datetime.now().strftime('%Y-%m-%d'),
                    'amount': 128000,
                    'sst_rate': '8%',
                    'reg_no': 'REG-123456',
                    'sst_id': 'SST-1234',
                    'address': '123 Jalan Kuala Lumpur'
                }
                yield f"data: {json.dumps({'step': 2, 'status': 'done', 'title': 'AI Extraction Complete', 'detail': 'Using fallback extraction (no API key)', 'extracted': extracted})}\n\n"
            else:
            ...
            # Step 5: Draft Email
            yield f"data: {json.dumps({'step': 5, 'status': 'running', 'title': 'Drafting Resolution Email', 'detail': 'Generating vendor communication...'})}\n\n"
            ...
            yield f"data: {json.dumps({'step': 6, 'status': 'error', 'title': 'Pipeline Failed', 'detail': str(e)})}\n\n"
